...
class SecurityView extends View {
				...
				${this._navView._generateMarkup()}
					<div class="col-12 col-md-8 col-lg-9">
						...
						<div class="${this._data.user.twoFactorAuth ? '' : 'd-none'}">
							...
							<button class="btn btn-outline-dark text-uppercase" data-tfa="disable">Deactivate Two-Factor Authentication</button>
						</div>
						<div class="${this._data.user.twoFactorAuth ? 'd-none' : ''}">
							<div class="mb-4">
							...
							<span class="d-inline-block mb-3" tabindex="0" ${this._data.user.email ? "" : "data-bs-toggle='popover'"} data-bs-trigger="hover focus" data-bs-content="missing email">
								<button class="btn btn-dark" type="button" ${this._data.user.email ? "" : "disabled"} data-tfa="send-code">
	...
	addHandlerTwoFactorAuth(handler) {
		document.addEventListener("click", e => {
			...
			if (!button || !button.dataset.tfa) return ;
			e.stopPropagation();
			const action = button.dataset.tfa;
			if (action === "send-code")
				button.closest("span").classList.toggle("d-none");
			handler(action);
		});
		document.addEventListener("submit", e => {
			...
			handler("submit", formData.get("code"));
		});
	}
};
